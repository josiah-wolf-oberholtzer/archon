...
def tensor_dict_from_dataframe(database_as_df):
    ...
    for k, v in database_as_df.items():
        v_nopitch = (
            v.drop(labels="pitch", axis=1)
            .drop(labels="rms", axis=1)
            .drop(labels="flat", axis=1)
        )
        column_list = list(v_nopitch.columns)
        tens_dict[k] = torch.tensor(
            v_nopitch.loc[:, column_list].to_numpy(dtype=np.float32)
        )
    return tens_dict
...
def process_input(input_dict):
    ...
    input_as_df = (
        input_as_df.drop(labels="pitch", axis=1)
        .drop(labels="rms", axis=1)
        .drop(labels="flat", axis=1)
        .drop(labels="variance", axis=1)
    )
    column_list = list(input_as_df.columns)
    in_t = torch.tensor(input_as_df.loc[:, column_list].to_numpy(dtype=np.float32))
    return pitch, variance, in_t


# use pytorch to find min. minkowski distance between input and db, return 25 closest matches
# note cuda is not enabled here yet
# (this is being tested on a 2015 macbook pro with only about 100 ms latency)
def closest_node(input_dataframe, database_dataframe, database_tensors, audiodir):
    ...
    if pitch not in database_dataframe:
        pitch = "unpitched"
    ...
    for i in range(25):
        if variety_ > float(variance):
            ...
            values, indices = torch.kthvalue(dist, dist.size(dim=0) - (i + 1))
        result.append(
            format_result(
                database_dataframe.get(pitch).iloc[indices.item()], pitch, audiodir
            )
        )
...
def format_result(sample, pitch, target_audiodir):
    ...
    if pitch != "unpitched":
        ...
        if float(cent) > 4000.0:
            return_dir = return_dir + "high_cent/"
        else:
            return_dir = return_dir + "low_cent/"
        if float(flat) > 0.01:
            return_dir = return_dir + "high_flat/"
        else:
            return_dir = return_dir + "low_flat/"
        if float(rolloff) > 8000:
            return_dir = return_dir + "high_rolloff/"
        else:
            return_dir = return_dir + "low_rollof/"
...
def osc_handler(unused_addr, constants, args):
    database_dataframe, database_tensors, audiodir, client = (
        constants[0],
        constants[1],
        constants[2],
        constants[3],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_port", type=int, default=5005, help="The port to listen on"
    )
    parser.add_argument(
        "--out_port", type=int, default=57120, help="The port to send to"
    )
    parser.add_argument(
        "--file",
        default="/Users/marek_orpheus/Desktop/ARCHON_db/analysis_500ms.json",
        help="Location of analysis file (json)",
    )
    parser.add_argument(
        "--audiodb",
        default="/Users/marek_orpheus/Desktop/ARCHON_db/",
        help="Location of audio database",
    )
    args = parser.parse_args()

    # sets up Supercollider client and test message.
    # if you don't see one in SC, start investigating
    client = SimpleUDPClient(args.ip, args.out_port)
    client.send_message(
        "/superInterface", "ACK: Testing Connection w/ Python Server, Hold for OK"
    )
    ...
    server = osc_server.ThreadingOSCUDPServer((args.ip, args.in_port), dispatcher)
